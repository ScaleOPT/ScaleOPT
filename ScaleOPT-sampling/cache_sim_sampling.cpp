...
ULL SHARDS_T_VALUE = 0;
ULL SHARDS_P_VALUE = 1 << 24;
...
bool is_sample(offset_t offset) {
  ULL hash[2];
  ULL T_i = 0;

  qhashmurmur3_128(&offset, sizeof(offset_t), hash);
  T_i = hash[1] & (SHARDS_P_VALUE - 1);

  return T_i < SHARDS_T_VALUE;
}
...
public:
  ...
  void set_total_access_count(index_t _total_access_count) { total_access_count = _total_access_count; }
...
private:
  ...
  void create_and_run_part_cache_with_affinity(int core_num, ULL cache_size) {
    ...
    auto p = new PartCache(this, core_num, (index_t)round((double)(cache_size)*SAMPLING_RATE), access_map.get_access_count(), false);
  ...
  void create_and_run_part_caches_with_affinity(int core_num, int cache_num_begin, int cache_num_end) {
    ...
    for(int i=cache_num_begin; i<cache_num_end; i++) {
      ...
      auto p = new PartCache(this, i, (index_t)round((double)(cache_size)*SAMPLING_RATE), access_map.get_access_count(), i<(cache_num_end-1));
...
public:
  ...
  caches(_part_cache_num, NULL), block_size(_block_size), thread_num(_thread_num) {
    ...
    if(min_size == max_size) { // Single size simulation
      ...
      caches[0] = new PartCache(this, 0, (index_t)round((double)(min_pcache_size)*SAMPLING_RATE), access_map.get_access_count(), false);
    ...
    if(thread_num==1) { // Single thread - multi size simulation
      caches[0] = new PartCache(this, 0,  (index_t)round((double)(min_size/block_size)*SAMPLING_RATE), access_map.get_access_count(), true);
      ...
      for(int i=1; i<part_cache_num-1; i++) {
        caches[i] = new PartCache(this, i, (index_t)round((double)(target_step_size/block_size)*SAMPLING_RATE), access_map.get_access_count(), true);
      ...
      caches[part_cache_num-1] = new PartCache(this, 0, (index_t)round((double)(last_step_size/block_size)*SAMPLING_RATE), access_map.get_access_count(), false);
    ...
    if(thread_num < part_cache_num) {
      ...
      caches[0] = new PartCache(this, 0, (index_t)round((double)(min_size/block_size)*SAMPLING_RATE), access_map.get_access_count(), pcache_num[0]>1);
      for(int i=1; i<pcache_num[0]; i++) {
          caches[i] = new PartCache(this, i, (index_t)round((double)(target_step_size/block_size)*SAMPLING_RATE), access_map.get_access_count(), i<(pcache_num[0]-1));
          caches[i-1]->set_next_level(caches[i]);
      }
      ...
      if(before_min_pcache_count > 1) {   
        caches[0] = new PartCache(this, 0, (index_t)round((double)(before_min_pcache_size/block_size)*SAMPLING_RATE), access_map.get_access_count(), false);
        ...
        caches[0] = new PartCache(this, 0, (index_t)round((double)(min_size/block_size)*SAMPLING_RATE), access_map.get_access_count(), false);
        pcache_sizes.push_back(min_size);
        target_size_caches.push_back(0);
      }
  ...
  void set_access_count_of_caches(index_t total_access_count) {
    for(auto c : caches) {
      c->set_total_access_count(total_access_count);
    }
  }
...
void calculate_optimal_hit_ratio(vector<offset_t> *access_log) {
  ...
  for(index_t i=0; i < access_log->size(); i++) {
    ...
    if(is_sample(cur_offset)) {
      total_access_count++;
      my_cache.access(cur_offset);
    }
  }
  my_cache.set_access_count_of_caches(total_access_count);
#ifdef THREAD_UPTIME_LOG
  ...
  cout << "Total reference after sampling: " << total_access_count << endl;
...
int main(int argc, char** argv) {
  if(argc < 8) {
    cout << "Usage: " << argv[0] << " [access log file]" << " [min cache size]" 
      << " [max cache size]" << " [step count]" << " [block size]" 
      << " [Thread #]" << " [sampling rate]" << endl;
    return -1;
  }
  ...
  SAMPLING_RATE = stod(argv[7]);
  SHARDS_T_VALUE = SAMPLING_RATE * SHARDS_P_VALUE;
